...
def plot_confusion_matrix(labels, predictions):
    ...
    cm = confusion_matrix(labels.cpu(), (predictions >= .5).cpu().int(), normalize='true')
    ...
    buf.seek(0)
    im = np.reshape(np.frombuffer(buf.getvalue(), dtype=np.uint8),
...
def plot_roc_curve(labels, predictions):
    ...
    RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=0).plot(ax=ax, color='cornflowerblue', lw=1.5)
...
def create_train_test_val(patient_data_dir, proportions, tabular_data=[]):
    ...
    patient_labels = pd.read_csv(os.path.join(patient_data_dir, 'patient_labels.csv'), dtype={'MRN':str})[['MRN', 'SLIDE_ID', 'LABEL'] + tabular_data]
    
    # Get a new dataframe of the unique MRNs
    patient_outcomes = []
    for mrn in patient_labels['MRN'].unique():
        patient_outcomes.append([mrn, patient_labels[patient_labels['MRN'] == mrn]['LABEL'].iloc[0]])
    patient_outcomes = pd.DataFrame(patient_outcomes, columns=['MRN', 'LABEL'])

    # Make a split on train and test with no shared patients
    train_df, test_df = train_test_split(patient_outcomes, test_size=test_prop, stratify=patient_outcomes['LABEL'])
    train_df = train_df.drop('LABEL', axis=1).merge(patient_labels, how='inner', on='MRN')
    ...
    train_df, val_df = train_test_split(train_df, test_size=val_prop/(train_prop + val_prop), stratify=train_df['LABEL'])
...
class TensorDataset(Dataset):
    def __init__(self, patient_data_file, image_data_dir, T_rbc, T_wbc):
        ...
        for path in patient_data['MRN'] + '_' + patient_data['SLIDE_ID'].astype(str):
            if self.T_rbc > 0:
                self.rbc_paths.append(os.path.join(image_data_dir, f'{path}/rbc/features/features_original.pt'))
            if self.T_wbc > 0:
                self.wbc_paths.append(os.path.join(image_data_dir, f'{path}/wbc/features/features_original.pt'))
    ...
    def __getitem__(self, idx):
        ...
        if self.T_rbc > 0:
            ...
            rbc = torch.nn.functional.pad(rbc, (0, 0, 0, max(0, self.T_rbc - rbc.shape[0])))
            rbc_indices = np.random.choice(np.arange(self.T_rbc), self.T_rbc, replace=False)
            rbc = rbc[rbc_indices]
        ...
        if self.T_wbc > 0:
            ...
            wbc = torch.nn.functional.pad(wbc, (0, 0, 0, max(0, self.T_wbc - wbc.shape[0])))
            wbc_indices = np.random.choice(np.arange(self.T_wbc), self.T_wbc, replace=False)
            wbc = wbc[wbc_indices]
...
class TensorDataModule(pl.LightningDataModule):
    def __init__(self, patient_data_dir, image_data_dir, batch_size, num_workers, T_rbc=121, T_wbc=114):
        super().__init__()
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.train_dataset = TensorDataset(os.path.join(patient_data_dir, "train.csv"), image_data_dir, T_rbc, T_wbc)
        self.val_dataset = TensorDataset(os.path.join(patient_data_dir, "val.csv"), image_data_dir, T_rbc, T_wbc)
        self.test_dataset = TensorDataset(os.path.join(patient_data_dir, "test.csv"), image_data_dir, T_rbc, T_wbc)
