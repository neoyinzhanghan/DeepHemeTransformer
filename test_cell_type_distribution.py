...
def extract_h5_data(result_folder, save_path, model, note=""):
    ...
    jpg_paths = [
        path for path in jpg_paths if "blurry" not in path and "YOLO" not in path
    ]
    ...
    # take argmax to get all_classes from all_class_probs (shape: (N, 23)) output shape: (N,)
    all_classes = all_class_probs.argmax(axis=1)

    # append all_classes to cell_class_tally
    cell_class_tally.extend(list(all_classes))
...
if __name__ == "__main__":
    ...
    for i, row in tqdm(metadata.iterrows(), total=len(metadata)):
        ...
        if num_slides > 250:
            ...
            plt.hist(cell_class_tally, bins=22)
            plt.xlabel("Class index")

            # save the plot at the h5 directory
            plt.savefig(os.path.join(save_dir, "class_tally_while_predict.png"))
