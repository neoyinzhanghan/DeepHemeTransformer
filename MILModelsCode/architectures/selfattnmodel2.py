...
class AggregateConcatenate(pl.LightningModule):
    ...
    #     # agg_embeddings is of shape B x T x agg_out_size
    #     # The aggregations will only be applied to the all the  queries.
    ...
    #     batch_representations.append(self.generalized_mean(batch_agg_embeddings, power=5.0))
    #     batch_representations.append(self.generalized_mean(batch_agg_embeddings, power=2.5))
    #     batch_representations.append(self.generalized_mean(batch_agg_embeddings, power=1.0))
    ...
    #     batch_representations.append(self.log_sum_exponentiation(batch_agg_embeddings, power=2.5))
    #     batch_representations.append(self.log_sum_exponentiation(batch_agg_embeddings, power=1.0))
    #     batch_representations.append(self.log_sum_exponentiation(batch_agg_embeddings, power=0.5))

    #     return torch.stack(batch_representations)  # TODO DEPRECATION
...
class MyMultiheadAttention(pl.LightningModule):
    ...
    def forward(self, input_tensor):
        ...
        full_output, attn = self.multihead_attention(
            query, key, value
        )  # B x (n+n+121+111) x attn_head_size, B x (2n+121+111) x (2n+121+111)
...
class MILSelfAttention(pl.LightningModule):
    ...
    def forward(self, cells):
        ...
        # concatenated_all has a shape of B x (n+n+121+114) x agg_out_size
