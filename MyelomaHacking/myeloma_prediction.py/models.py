...
class Classifier(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        feature_stack, logit_stack, non_padding_mask, diff_tensor, y = batch
        y_hat, _ = self.forward(feature_stack)
...
class MILSelfAttention(Classifier):
...
MODEL_DICT = {
    "myeloma": (2, (2048, 256, 128, "normal"), MILSelfAttention),
}
...
def load(project, experiment, version="best", checkpoint="best", **kwargs):
    ...
    checkpoint_path = (
        f"lightning_logs/{project}/{experiment}/{version}/checkpoints/{checkpoint}.ckpt"
    )
    model = model.load_from_checkpoint(
        checkpoint_path, num_classes=num_classes, args=args, **kwargs
    )
    model.eval()
    return model
