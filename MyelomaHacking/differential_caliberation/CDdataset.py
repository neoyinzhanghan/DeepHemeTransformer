...
class TensorStackDatasetV5(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # randomly sample between 0 and 1 with equal probability
        random_class = np.random.randint(0, 2)

        if random_class == 0:
            filtered_dx_data = self.dx_data[
                self.dx_data["dx"] == "Normal BMA"
            ].reset_index(drop=True)

            # randomly sample an index from the filtered dx data
            random_idx = np.random.randint(0, len(filtered_dx_data))

            accession_number = filtered_dx_data.loc[random_idx, "accession_number"]

        elif random_class == 1:
            filtered_dx_data = self.dx_data[
                self.dx_data["dx"] == "Plasma cell myeloma"
            ].reset_index(drop=True)
        ...
        corresponding_diff_data_row = self.diff_data[
            self.diff_data["accession_number"] == accession_number
        ].reset_index(drop=True)

        # assert exactly one row is returned
        assert (
            len(corresponding_diff_data_row) == 1
        ), f"More than one row returned when exactly one row is expected while searching for the accession number {accession_number}."

        accession_number = corresponding_diff_data_row.loc[0, "accession_number"]

        # get the result_dir_name from the corresponding diff data row
        result_dir_name = corresponding_diff_data_row.loc[0, "result_dir_name"]
        ...
        if dx == "Normal BMA":
            ...
            assert random_class == 0, f"Random class should be 0, but is {random_class}"
        elif dx == "Plasma cell myeloma":
            label = 1

            assert random_class == 1, f"Random class should be 1, but is {random_class}"
        else:
